...
local function apply_matugen_plugin_overrides()
    ...
    -- be reapplied when returning to the generated matugen theme.
...
function M.use_matugen()
    ...
    vim.g.current_theme = "matugen"
    state.set("theme", "matugen")

    -- Schedule once to run after colorscheme listeners from other plugins.
    vim.schedule(function()
        apply_matugen_plugin_overrides()
        sync_lualine_theme("pywal-nvim")
        refresh_bars()
    end)
end

M.use_pywal = M.use_matugen

function M.init()
    vim.g.current_theme = state.get("theme", "matugen")

    vim.api.nvim_create_user_command("ThemeMatugen", function()
        M.use_matugen()
    end, { desc = "Use matugen for all highlights" })

    vim.api.nvim_create_user_command("ThemePywal", function()
        M.use_matugen()
    end, { desc = "Use matugen-generated pywal compatibility colors" })
    ...
    vim.schedule(function()
        local startup_theme = state.get("theme", "matugen")
        if startup_theme == "matugen" or startup_theme == "pywal" then
            M.use_matugen()
        else
